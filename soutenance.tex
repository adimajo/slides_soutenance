...
Due to the acceptance mechanism, labels $y$ are not MCAR.
...
\uncover<3->{
Combining financing and credit-worthiness probability distributions, we get:
...
\vspace*{-0.4cm}
\begin{align*}
\bm{\theta}^\star, \q^\star = & \argmax_{\bm{\theta} \in \bm{\Theta}_{\textcolor{yellow}{\q}}, \textcolor{yellow}{\q \in \bm{Q}}} \mathbb{E}_{\bm{x},y} \ln p_{\bm{\theta}}(y | \q(\bm{x})) \\
\hat{\bm{\theta}}^{\text{BIC}}, \hat{\q}^{\text{BIC}} = & \argmin_{\bm{\theta} \in \bm{\Theta}_{\textcolor{yellow}{\q}}, \textcolor{yellow}{\q \in \bm{Q}}} \text{BIC}(\bm{\theta}; \bm{\mathbf{y}}_{\text{f}}, \q(\bm{\mathbf{x}}_{\text{f}}))
\end{align*}
\vspace*{-0.2cm}
\end{block}
...
\vspace*{-0.1cm}
\[ \hat{\q}_j^{\chi^2} = \argmax_{\q_j} \chi^2(\q_j(\mathbf{x}_\text{f}), \mathbf{y}_\text{f}) \stackrel{?}{\approx} \q^\star, \]
\vspace*{-0.1cm}
...
\[ (\q^\star,\bth^\star) = \argmin_{\textcolor{yellow}{\hat{\q} \in \{\hat{\q}^{(1)}, \ldots, \hat{\q}^{(\textit{iter})}\}},\bth \in \bm{\Theta}_{\textcolor{yellow}{\bm{m}}}} \text{BIC}(\hat{\bth}_{\hat{\q}}) \]
\end{block}
...
\[ (\hat{K}, \hat{\mathbf{c}}) = \argmax_{K,\mathbf{c}} \sum_{c=1}^K \text{BIC}((\bm{x}_i,y_i) | c_{i} = c;\bm{\theta}^{c}),\]
