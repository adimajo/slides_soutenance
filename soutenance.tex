...
\title[Soutenance]{Modèles de classification supervisée \\ et semi-supervisée \\ {\small Contrôle des pertes d’information inhérentes \\ au scoring en risque de crédit}}
...
\arrayrulecolor{OliveGreen}\hline
...
Contrary to semi-supervised learning~\cite{chapelle2009semi}, labels $y$ are not MCAR.
...
\uncover<3->{
Combining ``acceptance'' and credit-worthiness probability distributions, we get:
\[ p(y,z | \bm{x}) = p_{\bm{\theta}(\bm{\gamma})}(y | \bm{x}) p_{\bm{\phi}(\bm{\gamma})}(z | \bm{x}, y). \]
}
...
\item<2-> \textbf{MAR}: $\forall \: x,y,z, \; p(z| \bm{x},y) = p(z| \bm{x})$

$\rightarrow$ Acceptance is determined by an old score: $Z = \mathds{1}_{\{\bm{\theta}'X > \text{cut}\}}$.

\item<3-> \textbf{MNAR}: $\exists \: x,y,z, \; p(z| \bm{x},y) \neq p(z| \bm{x})$
...
We gathered 6 so-called Reject Inference methods from the literature~\cite{} that aim at re-injecting $\bm{\mathbf{x}}_{\nf}$ into the estimation procedure of $\hat{\bm{\theta}}$.
...
\[ (\hat{\bm{\theta}}, \hat{\bm{\alpha}}) = \argmax_{\bm{\theta}, \bm{\alpha}} \ell(\bm{\theta}, \bm{\alpha} ; \bm{x}, \bm{y}) = \sum_{i=1}^n \ln p_{\bm{\theta}}(y_i | \q_{\alpha}(\bm{x}_i)). \]
...
We have drastically restricted the search space to clever candidates $\hat{\q}^(1), \ldots, \hat{\q}^{(\text{iter})}$ resulting from the the gradient descent steps.
\[ (\q^\star,\bth^\star) = \argmin_{\textcolor{red}{\hat{\q} \in \{\hat{\q}^{(1)}, \ldots, \hat{\q}^{(\text{iter})}\}},\bth \in \bm{\Theta}_{\textcolor{red}{\bm{m}}}} \text{BIC}(\hat{\bth}_{\hat{\q}}) \]
\end{block}
...
\begin{align*}
p(y| \bqk ) & = \sum_{\bm{\delta} \in \{0,1\}^\frac{d(d-1)}{2}} p(y | \bqk, \bm{\delta}) p(\bm{\delta}) \\
p(\bm{\delta} | \bqk,y) & \propto \exp(-\text{BIC}[\bm{\delta}]/2) \textcolor{red}{\cancel{p(\bm{\delta})}} \hspace{1cm} p(\delta_{p,q}) = \frac{1}{2}
\end{align*}
}
\uncover<5->{
...
\[ (\hat{c}, \hat{K}) = \argmax_{c,K} \sum_{c=1}^K \text{BIC}(\hat{\bm{\theta}}^c),\]
